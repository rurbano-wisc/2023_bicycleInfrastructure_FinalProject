...
(function () {
    ...
    function getMetroAreaBoundaryData() {
        ...
        var osm150ftStyle = {
            //fillColor: "#A65E44",
            color: "#453dcf", //random color - fix it later
            weight: 1,
            opacity: 1,
            fillOpacity: 0
        };
        ...
        var osmStyle = {
            //fillColor: "#A65E44",
            color: "#123acf", //random color - fix it later
            weight: 1,
            opacity: 1,
            fillOpacity: 0
        };
        ...
        fetch("data/OSM_Nevada.geojson")
            ...
            .then(function (json) {
                osmNev = L.geoJSON(json, osm150ftStyle)//.addTo(map); took this out so that it wouldn't be added to the map when it first loads, then it will turn on when the zoom reaches level 6
        ...
        fetch("data/OSM_California.geojson")
            ...
            .then(function (json) {
                osmCal = L.geoJSON(json, osm150ftStyle)//.addTo(map); took this out so that it wouldn't be added to the map when it first loads, then it will turn on when the zoom reaches level 6
